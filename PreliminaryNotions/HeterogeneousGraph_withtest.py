...
for i in range(1,numofep):
    ...
    acc_HeteroGNN_changinglayer.append(test())

loss_HeteroGNN_replicatedlayerConf = []
acc_HeteroGNN_replicatedlayerConf = []
...
for i in range(1,numofep):
    ...
    acc_HeteroGNN_replicatedlayerConf.append(test())

loss_HeteroGNN_iterativeLayer = []
acc_HeteroGNN_iterativeLayer = []
...
for i in range(1,numofep):
    ...
    acc_HeteroGNN_iterativeLayer.append(test())

loss_HGT_iterative = []
acc_HGT_iterative = []
model = HGT_iterative(hidden_channels=64, hidden_channels2=32, hidden_channels3=16, out_channels=num_of_class, num_heads=2)
optimizer = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=5e-4)  # Define optimizer.
model.to(device)
for i in range(1,numofep):
    print(f"Epoch {i}")
    ...
    acc_HGT_iterative.append(train())

loss_HGT_fixed = []
acc_HGT_fixed = []
model = HGT_fixed(hidden_channels=64, out_channels=num_of_class, num_heads=2, num_layers=2)
optimizer = torch.optim.Adam(model.parameters(), lr=0.01, weight_decay=5e-4)  # Define optimizer.
model.to(device)
for i in range(1,numofep):
    print(f"Epoch {i}")
...
plt.plot(acc_HeteroGNN_changinglayer, label="acc_HeteroGNN_changinglayer")
plt.plot(acc_HeteroGNN_replicatedlayerConf, label="acc_HeteroGNN_replicatedlayerConf")
plt.plot(acc_HeteroGNN_iterativeLayer, label="acc_HeteroGNN_iterativeLayer")
plt.plot(acc_HGT_iterative, label="acc_HGT_iterative")
plt.plot(acc_HGT_fixed, label="acc_HGT_fixed")
plt.legend()
plt.savefig("acc.png")
